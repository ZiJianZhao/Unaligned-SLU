...
def model_opts(parser):
    ...
    parser.add_argument('-wcn_emb_type', default='swsr', type=str,
                       help="wcn embedding types")
...
def make_model(opt):
    model = SLUSystem(opt.wcn_emb_type,
            opt.enc_word_vocab_size, opt.dec_word_vocab_size, 
            opt.act_vocab_size, opt.slot_vocab_size,
            opt.emb_dim, opt.hid_dim, opt.dropout, opt.num_layers)
    return model
...
def train(opt):
    ...
    if opt.load_word_emb:
        emb = read_emb(opt.memory['enc2idx'], opt.emb_dim)
        model.encoder.embeddings.init_weight_from_pre_emb(emb, opt.fix_word_emb)
    ...
    # training procedure
    if opt.task == 'slu':
        data_iter = SLUDataset(opt.data_root + 'train', opt.memory, opt.cuda, True)
        trainer = SLUTrainer(model, (stc_criterion, nll_criterion), optimizer, opt.logger, cuda=opt.cuda)
    elif opt.task == 'act':
        data_iter = ActDataset(opt.data_root + 'train', opt.memory, opt.cuda, True)
        trainer = ActTrainer(model, stc_criterion, optimizer, opt.logger, cuda=opt.cuda)
    elif opt.task == 'slot':
        data_iter = SlotDataset(opt.data_root + 'train', opt.memory, opt.cuda, True)
        trainer = SlotTrainer(model, stc_criterion, optimizer, opt.logger, cuda=opt.cuda)
    elif opt.task == 'value':
        data_iter = ValueDataset(opt.data_root + 'train', opt.memory, opt.cuda, True)
...
def test(opt):
    ...
    for session in sessions:
        ...
        for turn in session['turns']:
            ...
            classes = decode_slu(model, asr_hyps, opt.memory, opt.cuda)
...
def error(opt):
    ...
    for pair in datas:
        ...
        if opt.task == 'act':
            ...
            pred_classes = decode_slot(model, cnet, class_string, opt.memory, opt.cuda)
        elif opt.task == 'value':
            gold_classes = ValueDataset.class_info(class_string)
            pred_classes = decode_value(model, cnet, class_string, opt.memory, opt.cuda)
        elif opt.task == 'slu':
