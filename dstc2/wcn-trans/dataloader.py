...
def process_cn_example(cnet, word2idx):
    
    new_cnet = copy.deepcopy(cnet)
    ct = {'arcs': [{"score":-0.0001, 'word': '[CLS]'}], 'end': cnet[0]['end'], 'start': cnet[0]['start']}
    new_cnet.insert(0, ct)
    cnet = new_cnet

    data = [ [] for i in range(len(cnet)) ]
    weight = [ [] for i in range(len(cnet)) ]
    words = []
    for i in range(len(cnet)):
        ct = cnet[i]
        word = sorted(ct['arcs'], key=lambda x: x['score'], reverse=True)[0]['word']
        word = process_word(word)
        words.append(word)
        for arc in ct['arcs']:
            word = process_word(arc['word'])
            if word in word2idx:
                data[i].append(word2idx.get(word))
            else:
                data[i].append(word2idx.get(Constants.UNK_WORD))
            weight[i].append(math.exp(arc['score']))
    if sum([len(d) for d in data]) == 0:
        return None
    else:
        max_arc_num = max([len(arc) for arc in data])
        for i in range(len(data)):
            data[i].extend([Constants.PAD for i in range(max_arc_num-len(data[i]))])
            weight[i].extend([0 for i in range(max_arc_num-len(weight[i]))])
        data = torch.from_numpy(np.asarray(data, dtype='int64'))
        weight = torch.from_numpy(np.asarray(weight, dtype='float32'))
        if data.size(0) >= 500:
            print('1')
            return None
        return data, weight, words
...
class SLUDataset(object):
    ...
    def __init__(self, filename, memory, cuda, epoch_shuffle):
        ...
        self.datas = self.read_file(filename, self.memory)
    ...
    @staticmethod
    def data_info(cnet, memory, cuda):

        data, weight, words = process_cn_example(cnet, memory['enc2idx'])

        word2idx = memory['dec2idx']
        ids, oov_list = seq2extend_ids(words, word2idx)
    ...
    def __next__(self):
        ...
        pair = self.datas[self.indices[self.idx]]
        self.idx += 1
        ...
        data, lengths, extra_zeros, enc_batch_extend_vocab_idx, oov_list = \
                self.data_info(cnet, self.memory, self.cuda)
...
class ActDataset(object):
    ...
    def __next__(self):
        ...
        data, lengths = self.data_info(cnet, self.memory, self.cuda)
        ...
        for pair in pairs:
            ...
            for string in classes:
                ...
                if act == pre_act:
                    ...
                    tmp_pair = copy.deepcopy(pair)
                    tmp_pair['label'] = class_string
                    datas.append(tmp_pair)
                    tmp = [string]
            if len(tmp) != 0:
                class_string = ';'.join(tmp)
                tmp_pair = copy.deepcopy(pair)
