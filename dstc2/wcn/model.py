...
from xslu.modules import Embedding, EncoderRNN, Attention, \
        SWSREmbedding, MWSREmbedding, SAVGEmbedding, CAVGEmbedding
import xslu.Constants as Constants

class SLUSystem(nn.Module):
    """Embeddings for SLU"""

    def __init__(self, wcn_emb_type, enc_word_vocab_size, dec_word_vocab_size, act_vocab_size, slot_vocab_size, 
        ...
        if wcn_emb_type == 'swsr':
            self.enc_word_emb = SWSREmbedding(enc_word_vocab_size, emb_dim, Constants.PAD, dropout)
        elif wcn_emb_type == 'mwsr':
            self.enc_word_emb = MWSREmbedding(enc_word_vocab_size, emb_dim, Constants.PAD, dropout)
        elif wcn_emb_type == 'savg':
            self.enc_word_emb = SAVGEmbedding(enc_word_vocab_size, emb_dim, Constants.PAD, dropout)
        elif wcn_emb_type == 'cavg':
            self.enc_word_emb = CAVGEmbedding(enc_word_vocab_size, emb_dim, Constants.PAD, dropout)
        else:
