...
from xslu.modules.utils import aeq, sequence_mask
...
class Attention(nn.Module):
    ...
    * Attention is all you need (scaled dot-product attention):
    ...
    def forward(self, qry, ctx_keys, ctx_values, ctx_lengths=None):
        """Compute attention scores
        Args:
            qry (tensor): batch_size * qry_len * qry_dim
            ctx_keys (tensor): batch_size * ctx_len * ctx_dim
            ctx_values (tensor): batch_size * ctx_len * ctx_dim
        ...
        attn_ctx = torch.bmm(attn, ctx_values) # batch_size * qry_len * ctx_dim
