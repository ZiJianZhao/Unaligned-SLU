...
def beam_search(model, act_slot, extra_zeros, enc_batch_extend_vocab_idx,
        init_state, enc_outputs, enc_lengths, memory, oov_list, cuda,
        beam_size = 7, max_steps=25):
    ...
    while (step < max_steps) and (len(results) < beam_size):
        ...
        def get_id(idx):
            if idx < len(memory['dec2idx']):
                return idx
            else:
                word = oov_list[idx - len(memory['idx2dec'])]
                if word in memory['enc2idx']:
                    idx = memory['enc2idx'][word]
                else:
                    idx = Constants.UNK
                return idx

        latest_tokens = [get_id(t) for t in latest_tokens]
