...
class SLUDataset(object):
    ...
    def label_info(class_string, memory, enc_oov_list, cuda):
        ...
        for (i, (key, value)) in enumerate(dic.items()):
            ...
            for v in value:
                if v in slot2idx:
                    ...
                    slot_ids[i].append(Constants.PAD)
        ...
        for (i, (key, value)) in enumerate(dic.items()):
            ...
            if act_slot_lis[1] in slot2idx:
                act_slot_ids[i].append(slot2idx[act_slot_lis[1]])
            else:
                act_slot_ids[i].append(Constants.PAD)
            inp_ids = value2ids(value.strip().split(), word2idx)
            out_ids = value2extend_ids(value.strip().split(), value2idx, enc_oov_list)
...
class ValueDataset(object):
    ...
    def label_info(class_string, memory, enc_oov_list, cuda):
        ...
        ## Note: one act-slot pair can only correspoding to one value
        if slot in slot2idx:
            slot_id = slot2idx[slot]
        else:
            slot_id = Constants.PAD

        act_slot_ids = torch.tensor([act2idx[act], slot_id]).view(1, 2)
        inp_ids = value2ids(value.strip().split(), word2idx)
        out_ids = value2extend_ids(value.strip().split(), value2idx, enc_oov_list)
