...
root_dir = '../../'
glove_path = '../../../NLPData/final.nnet.80g.wordvec.lm'
...
class Memory4MapHD(object):

    """Class used to process Chinese maps texts and save the processed contents.
    ...
    def __init__(self):
        super(Memory4MapHD, self).__init__()
    ...
    def build_save_memory(self, train_file, class_file, memory_file):
        ...
        self.judge_word_vocab_in_glove(word2idx)
        ...
        act2idx, slot2idx = self.build_class_vocab(class_path)
        ...
        # -----------------------------------------------------
        value2idx = self.build_value_vocab(train_path)
    ...
    def class_info(self):
        single_acts = ['ack', 'affirm', 'bye', 'hello', 'negate', 'repeat', 
            'reqalts', 'reqmore', 'restart', 'thankyou', 'findalts', 'affirm', 'askrepeat', 'chat']
        double_acts = ['request']
    ...
    def judge_word_vocab_in_glove(self, word2idx):
        ...
        for word in word2idx:
            if word not in words:
                num += 1
        print('Word vocab not in glove: {}'.format(num))
        print('==========================================')
    ...
    def build_value_vocab(self, filename):
        with codecs.open(filename, 'r') as f:
            lines = f.readlines()
        values = []
        for line in lines:
        ...
        print(value2idx)
        print('value2idx size: {}'.format(len(value2idx)))
        return value2idx
