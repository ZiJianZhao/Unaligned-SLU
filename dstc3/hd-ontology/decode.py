...
def correct_value(slot, value,
    ontology_path='/slfs1/users/zjz17/SLU/dstc3/seed/scripts/config/ontology_dstc3.json'):

    dic = json.loads(open(ontology_path).read())['informable']
    dic['this'] =  ['dontcare']

    if slot not in dic:
        return value
    else:
        values = dic[slot] + ['dontcare']

    lis = value.strip().split()
    tmp = list(set(lis))
    tmp.sort(key=lis.index)
    ...
    for v in values:
        similarity = difflib.SequenceMatcher(None, value, v).ratio()
        #similarity = Levenshtein.ratio(value, v)
...
def judge_act_slot(act, slot):
    if act in ['confirm', 'inform', 'deny']:
        if slot not in ['this', 'name', 'hastv', 'food', 'childrenallowed',
            'area', 'hasinternet', 'near', 'pricerange', 'type']:
            return False
    return True
...
def decode_slu(model, utterance, memory, cuda):
    ...
    for act in remain_acts:
        ...
        if act[1] in memory['double_acts']:
            ...
            for slot in slot_pairs:
                #if slot[1] != 'pad' and judge_act_slot(act[1], slot[1]):
                if slot[1] != 'pad':
