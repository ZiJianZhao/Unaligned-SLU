...
class SLUDataset(object):
    ...
    def data_info(utterance, memory, cuda):
        ...
        lis = process_sent(utterance)
        if len(lis) == 0:
            raise Exception("Input utterance can not be empty string")
    ...
    def label_info(class_string, memory, enc_oov_list, cuda):
        ...
        if len(lis) == 0:
            ...
            acts = [label.strip().split('-', 2)[0] for label in lis]
        ...
        lis = [string for string in lis if len(string.strip().split('-', 2)) > 1]
        if len(lis) == 0:
        ...
        lis = [string for string in lis if len(string.strip().split('-', 2)) > 2]
        if len(lis) == 0:
    ...
    def __next__(self):
        ...
        if self.epoch_shuffle:
            pad_data = self.judge_utt_label(utterance, class_string, self.memory, self.cuda)
            if pad_data is not None and random.random() > 0.5:
                data = pad_data
    ...
    def read_file(filename):
        ...
        for line in lines:
            ...
            if class_string.strip() == '':
                continue
            classes = class_string.strip().split(';')
            classes = [string for string in classes if len(string.strip().split('-', 2)) >= 2]
            classes = sorted(classes)
            if len(classes) == 0:
                continue

            pre_act = classes[0].strip().split('-', 2)[0]
            tmp = []
            for string in classes:
                act = string.strip().split('-', 2)[0]
                if act == pre_act:
                    tmp.append(string)
                else:
...
class ValueDataset(object):
    ...
    @staticmethod
    def class_info(class_string):
        values = [ class_string.strip().split('-', 2)[2].strip() ]
        return values
    ...
    def label_info(class_string, memory, enc_oov_list, cuda):
        ...
        act, slot, value = tuple(class_string.strip().split('-', 2))
