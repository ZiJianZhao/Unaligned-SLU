...
train_data_dir = root_dir + 'data/'
valid_data_dir = root_dir + 'data/'
seed_data_dir = root_dir + 'data/'
test_data_dir = root_dir + 'data/'
train_flist_path = root_dir + 'scripts/config/dstc3_st_train.flist'
valid_flist_path = root_dir + 'scripts/config/dstc3_st_valid.flist'
seed_flist_path = root_dir + 'scripts/config/dstc3_st_seed.flist'
test_flist_path = root_dir + 'scripts/config/dstc3_st_test.flist'
...
def get_classes_from_flist(data_dir, flist_path):
...
def get_classes(save_dir):
    ...
    classes_train = get_classes_from_flist(train_data_dir, train_flist_path)
    with open(class_train_save_path, 'w') as f:
        for cls in classes_train:
            f.write('{}\n'.format(cls))

    class_seed_save_path = save_dir + 'class.seed'
    classes_seed = get_classes_from_flist(seed_data_dir, seed_flist_path)
    with open(class_seed_save_path, 'w') as f:
        for cls in classes_seed:
            f.write('{}\n'.format(cls))
...
def get_all_pairs(task):
    ...
    seed_pairs = get_pairs(seed_data_dir, seed_flist_path, save_dir+'seed', task)
    if task.startswith('nbest'):
        count_nbest(seed_pairs, 'Seed')
    else:
        count(seed_pairs, 'Seed')
